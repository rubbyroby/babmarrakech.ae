        ...
        <div class="product-card home-page-products-card" style="background-color: white;">
            ...
            @else
                @if ($product->productLabels->count())
                    ...
                    <div class="product_badges-tag" @if ($label->color) style="background-color: {{ $label->color }} !important;color:#fff !important; border-radius: 8px; top: 4px; left: 4px; font-size: 1rem; box-shadow: 0 3px 8px rgba(0,0,0,.05);" @endif>{{ $label->name }}</div>
                    @endforeach
                @else
                    @if ($product->front_sale_price !== $product->price)
                            <div class="product_badges-tag" style="top: 4px; left: 4px; font-size: 14px; font-weight: bold;">{{ get_sale_percentage($product->price, $product->front_sale_price) }}</div>
                    @endif
                @endif
            @endif
